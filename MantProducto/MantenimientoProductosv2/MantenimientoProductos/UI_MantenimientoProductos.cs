...
namespace MantenimientoProductos
{
    public partial class UI_MantenimientoProductos : Form
    {
        ...
        private void buttonManteinence_Click(object sender, EventArgs e)
        {
            ...
            {
                UI_Ingreso_ModificacionProducto form = new UI_Ingreso_ModificacionProducto(tipoMov, codigoACambiar);
                form.TopLevel = false;
        ...
        void refresh()
        {
            ...
            try
            {
                DataSet ds = cpd.LlenarGrid("" +
                    ...
                    "tbl_producto.fuerza as FUERZA, tbl_producto.temperatura as 'TEMP.', tbl_producto.rpm as RPM, tbl_producto.hz as Hz, tbl_producto.precio as PRECIO, " +
            ...
            try
            {
                labelInStock.Text = cpd.ExtraerCodigo2("COUNT(tbl_ProductoBodega.Cantidad)", "bd_Ferreteria.tbl_Producto, bd_Ferreteria.tbl_ProductoBodega, bd_Ferreteria.tbl_Bodega",
                    "tbl_ProductoBodega.tbl_Producto_id = tbl_Producto.id AND tbl_ProductoBodega.tbl_Bodega_id = tbl_Bodega.id AND tbl_Producto.status = 1 AND tbl_Bodega.status = 1 AND tbl_Bodega.NombreBodega ", 
                    "'Stock'");
            }catch(Exception ex)
            {
                MessageBox.Show(ex.ToString(), "Error al Cargar los Datos", MessageBoxButtons.OK, MessageBoxIcon.Error);
        ...
        private void buttonDelete_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    cpd.ModificarDatos("tbl_producto", "status = 0", "id", codigoACambiar);
